...
All metrics that are not system level, are available on a Grafana dashboard. For the webservers, these metrics are generated by the Prometheus library for C#, and collected by a Prometheus instance running on the server.
